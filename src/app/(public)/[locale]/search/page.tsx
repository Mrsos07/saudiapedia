...
import { isLocale, matchesSection, searchEntries, sectionLabels, type Section } from '@/lib/encyclopedia';
...
export default async function Search({ params, searchParams }: Props) {
  ...
  const requestedSection = ['notable-figures', 'rulers'].includes(input.section ?? '') ? 'people' : input.section;
  const section = typeof requestedSection === 'string' && Object.hasOwn(sectionLabels, requestedSection) ? requestedSection as Section : '';
