...
import { brand, intro, pageMetadata } from '@/lib/site';
...
export default async function Home({ params }: Props) {
  ...
  const people = entries.filter(e => matchesSection(e, 'people'));
  const featuredPeople = [...people.filter(e => e.kind === 'ruler').slice(0, 3), ...people.filter(e => e.kind !== 'ruler')].slice(0, 4);
  return <main id="main-content">
    ...
    <section className="container section"><SectionHeading locale={locale} eyebrow={ar ? 'الإنسان الذي يصنع التاريخ' : 'THE PEOPLE BEHIND THE HISTORY'} title={seedSectionLabels.people[locale]} text={ar ? 'تعرّف على حكّام المملكة والشخصيات التي أسهمت في تاريخها وعلمها وثقافتها، في قسم واحد.' : 'Explore the rulers and people who shaped the Kingdom’s history, scholarship and culture, together in one section.'} href={`/${locale}/notable-figures`} action={ar ? 'جميع الشخصيات البارزة' : 'All notable figures'} /><div className="people-grid">{featuredPeople.map((entry, index) => <article className="person-card" key={`${entry.section}/${entry.slug}`}><span className="person-number">0{index + 1}</span><span className="person-period">{entry.period}</span>{entry.kind === 'ruler' && <><div className="person-portrait"><Image src={entry.image} alt={entry.imageAlt[locale]} fill sizes="(max-width: 640px) 80vw, (max-width: 900px) 40vw, 22vw" unoptimized={entry.image.startsWith('/api/')} /></div><PhotoCredit image={entry.image} locale={locale} credit={entry.imageCredit} /></>}<h3><Link href={entryPath(entry, locale)}>{entry.title[locale]}</Link></h3><p>{entry.summary[locale]}</p><Link className="text-link" href={entryPath(entry, locale)}>{ar ? 'اقرأ السيرة' : 'Read biography'} <Arrow locale={locale} /></Link></article>)}</div>{!people.length && <p className="no-results">{ar ? 'ستظهر السير هنا بعد اعتمادها باللغتين في لوحة التحرير.' : 'Biographies appear here once approved in both languages.'}</p>}</section>
